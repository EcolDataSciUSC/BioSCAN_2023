...
get.y.val.main.all <- function(sims.mat, ev, my.data){
  ...
  if(ev == "GDD1"){
    t <- seq(from=min(my.data[[1]]$GDD_Prev_Month),  
             to=  max(my.data[[1]]$GDD_Prev_Month),
             length.out=1000)
    p <- mean(my.data[[1]]$RH)
    ...
    t <- mean(my.data[[1]]$GDD_Prev_Month)
...
get.y.val <- function(sims.mat, ss, tt, pp, vv, ii) {
  ...
  chains <- expit(sims.mat[,'mu.psi.0'] +
                    ...
                    sims.mat[,sprintf('psi.GDD1[%s]', ss)]    * tt +
                    sims.mat[,'mu.psi.GDD.quad']    * (tt)^2+
