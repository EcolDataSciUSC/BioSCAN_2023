model.jags <- function() {
  ...
  ## Species-specific slopes to RH
  
  mu.psi.RH ~ dnorm(0,0.01)
  sigma.psi.RH ~ dunif(0,10)
  tau.psi.RH <- 1/(sigma.psi.RH*sigma.psi.RH)
  
  for(sp in 1:nsp){
    psi.RH[sp] ~ dnorm(mu.psi.RH, tau.psi.RH)
  }
