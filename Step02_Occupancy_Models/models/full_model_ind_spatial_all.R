model.jags <- function() {
  ...
  ## Species specific slopes (i.e. random effect) to diurnal temp range
  mu.psi.temp.diurnal ~ dnorm(0,0.01)
  sigma.psi.temp.diurnal ~ dunif(0,10)
  ...
  ## Species-specific slopes to land value
  
  mu.psi.value ~ dnorm(0,0.01)
  sigma.psi.value ~ dunif(0,10)
  ...
  ## Species-specific slopes to % impervious surface
  
  mu.psi.imperv ~ dnorm(0,0.01)
  sigma.psi.imperv ~ dunif(0,10)
  ...
  ## Species-specific slopes to ocean effect
  
  mu.psi.ocean.effect ~ dnorm(0,0.01)
  sigma.psi.ocean.effect ~ dunif(0,10)
  ...
  for(ind in 1:nind) {
          ...
          logit(psi[ind]) <- #has to be in same order as array
            ...
            psi.temp.diurnal[sp[ind]]*Temp_diurnal[site[ind]] +
            psi.value[sp[ind]]*landvalue[site[ind]] +
            psi.imperv[sp[ind]]*imperv[site[ind]] +
            psi.ocean.effect[sp[ind]]*ocean_effect[site[ind]] +
